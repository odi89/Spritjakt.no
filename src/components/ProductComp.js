    ...
    render() {
        ...
        if (product.Stock.Stores.length > 0 && selectedStore !== "0") {
            var store = product
                .Stock
                .Stores
                .find(s => s.name === selectedStore);
            stock = store.stockInfo.stockLevel > 99
                ? "99+"
        ...
        return (
            ...
            <li
                id={product.Id}
                className={"ProductComp " + (priceIsLower
                ? "price_lowered"
                : "price_raised")}
                ...
                <button
                    ...
                    onClick={() => this.props.setGraph(product.Id, this.productButton)}>{product.Name}</button>
                <div className="product_img" style={background}></div>
                {(showDiff && product.Discount !== "0.0") && <span className="percentage_change">{(priceIsLower
                        ? ""
                        : "+") + product.Discount}%</span>
}
                <span className="change_time">Endret
                    <br/> {lastChangedDate
                        .toISOString()
                        .slice(0, 10)}</span>
                <div className="product_details">
                    ...
                    <span className="stock" title="Lagerstatus">{stock}
                        <FontAwesomeIcon icon={faBoxes}/></span>
                    <span className="volume">{product.Volume * 100}
                        cl</span>
                    <span className="alcohol">Alk. {product.Alcohol}%</span>
                    <span className="price">Kr. {product.LatestPrice}</span>
                    {showDiff && <span className="old_price">Kr. {product.ComparingPrice}</span>
}
                    {product.Description && <span className="description">
                        <p className="colour">
                            <span>Farge</span>
                            {product.Description.characteristics.colour}</p>
                        <p className="odour">
                            <span>Lukt</span>
                            {product.Description.characteristics.odour}</p>
                        <p className="taste">
                            <span>Smak</span>
                            {product.Description.characteristics.taste}</p>
                    </span>
}
                </div>
            </li>
