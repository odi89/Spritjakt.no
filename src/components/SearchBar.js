...
class SearchBar extends React.Component {
   ...
   async SearchProducts(searchString) {
       ...
       this.setState({
        ...
        firebase.analytics().logEvent('product_search', {
            value: searchString
          });
    }
    ...
    displayProducts = () => {
        ...
        this.state.loadedProducts.map(p => {
            list.push(<ProductComp showDiff={false} key={p.Id} product={p} setGraph={this.setGraph.bind(this)} />)
        });
        if(list.length === 0){
            list.push( <p style={{"textAlign" :" center", width: "100%", position:"absolute"}}>Fant ikke noe.<br />Men så er denne søkefunksjonen shit også da...</p>);
        }
        return list;
      }
    ...
    return (
        <div className="SearchBar">
            ...
            <label> <span>Søk</span><input className="searchbox" type="text" placeholder="Søk på produktnavn" value={this.state.searchString} onChange={this.handleChange} />
            </label>
            {this.state.searchString.length >= 3 &&
            <div className="wrapper">
                <button className="close" onClick={() => this.setState({searchString: ""})} > <FontAwesomeIcon icon={faTimes} /></button>
