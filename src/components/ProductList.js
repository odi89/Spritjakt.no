...
class ProductList extends React.Component {
    ...
    async updateProductResults(timeSpan) {
        ...
        Object
            .keys(productTypes)
            .map(ptkey => (productTypes[ptkey].count = {
                ["0"]: 0
            }));
        ...
        let selectedStore = stores[this.state.selectedStore]
        ...
        let showAllresults = selectedTypes.length > 0
            ? false
            : true;

        SortArray(sortedStores, {by: "name"});

        stores["0"] = {
            name: "Alle butikker",
            count: loadedProducts.length
        }
        sortedStores.unshift({name: "Alle butikker", id: "0"});
        ...
        this.setState({
            ...
            productResultCount: showAllresults
                ? loadedProducts.length
                : filteredResultCount,
            showAllresults: selectedTypes.length > 0
                ? false
                : true
        });
    ...
    selectAllTypes = () => {
        let productTypes = this.state.productTypes;
        Object
            .keys(this.state.productTypes)
            .map(pt => (productTypes[pt].state = false));
        this.setState({productTypes: productTypes, showAllresults: true, productResultCount: this.state.loadedProducts.length});
    }
    ...
    handleFilterClick = (isSelected, name) => {
        ...
        firebase
            .analytics()
            .logEvent('filter_click', {value: productTypes[name]});
    ...
    updateFilter = (selectedStore, productTypes = this.state.productTypes) => {
        ...
        let selectedTypes = Object
            .keys(productTypes)
            .filter(pt => (productTypes[pt].state && productTypes[pt].count[selectedStore]));
        ...
        this.setState({productTypes: productTypes, showAllresults: showAllresults, productResultCount: productResultCount, page: 1});
    ...
    displayProducts = () => {
        ...
        for (let i = startPoint; i < this.state.loadedProducts.length; i++) {
            ...
            if ((displayedProducts < this.state.pageSize) && (this.state.showAllresults || this.state.productTypes[p.SubType].state) && (this.state.selectedStore === "0" || p.Stock.Stores.find(s => s.name == this.state.selectedStore))) {
                displayedProducts++;
                list.push(<ProductComp
                    key={p.Id}
                    showDiff={true}
                    product={p}
            ...
            .map(ptKey => {
                if (this.state.productTypes[ptKey].count[this.state.selectedStore] > 0) 
                    list.push(<ProductType
                        ...
                        .bind(this)}
                        name={ptKey}
                        productType={this.state.productTypes[ptKey]}/>)
            });
        return list;
    }
    ...
    renderStoreOptions = () => {
        ...
        this
            ...
            .sortedStores
            .map(store => (list.push(
                <option key={store.id} value={store.id}>{store.name + " (" + this.state.stores[store.id].count + ")"}</option>
            )));
        return list;
    }
    ...
    setPage = (page) => {
        ...
        Scroll
            .animateScroll
            .scrollTo(this.productList.current.offsetTop - 300)
    }
    render() {
        ...
        return (
            <div key="Productlist" className="main">
                ...
                <div className="before_products">
                    <div className="nav">
                        <div
                            className={"filter " + (this.state.filterVisibility
                            ? "active"
                            : "inactive")}>
                            <button
                                ...
                                firebase
                                    .analytics()
                                    .logEvent('filter_toggle_handheld');
                            }}>
                            ...
                            <fieldset disabled={!this.state.filterVisibility && isMobile}>
                                ...
                                <button
                                    ...
                                    className={"clickable resetFilter show " + (this.state.showAllresults
                                    ? "inactive"
                                    : "active")}
                ...
                <CSSTransition
                    ...
                    <div>
                        {this.state.highlightedProduct && <div className="priceGraphWrapper">
                ...
                <div
                    className={"filter_backdrop " + (this.state.filterVisibility
                    ? "active"
                    : "inactive")}
                    onClick={() => this.setState({filterVisibility: false})}></div>
            </div>
        );
    }
}
