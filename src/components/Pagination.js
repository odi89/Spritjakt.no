...
class Pagination extends React.Component {
    ...
    renderPageButtons = () => {
        ...
        for (let i = 1; i <= Math.ceil(this.props.total/this.props.pageSize); i++) {
            list.push(<PageButton key={"page-"+i} page={i} isSelected={this.props.page==i} setPage={this.setPage.bind(this)} />);
        }
        return list;
    }
    render(){
    ...
    return (
      <nav className="Pagination">
        ...
        <span>Viser {productsShowingtext} av {total} produkter</span>
      </nav>
    );
  }
}
